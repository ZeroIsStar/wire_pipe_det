    ...
    : Node("wire_andpipe_detection_node")
{
    ...
    this->declare_parameter<double>("fx", 925.68391);       
    this->declare_parameter<double>("fy", 925.96193);       
    this->declare_parameter<double>("cx", 647.85874);       
    this->declare_parameter<double>("cy", 464.19914);       
    this->declare_parameter<int>("camera_width", 1280);     
    this->declare_parameter<int>("camera_height", 1024);    
    this->declare_parameter<double>("h_fov_rad", 1.289);
    this->declare_parameter<double>("v_fov_rad", 1.136);
    this->declare_parameter<double>("camera_x", 0.0);
    this->declare_parameter<double>("camera_y", 0.0);
    this->declare_parameter<double>("camera_z", 0.925);
    this->declare_parameter<double>("camera_pitch", -0.01745);
    this->declare_parameter<double>("camera_yaw", 0.0);
    this->declare_parameter<double>("camera_roll", 0.0);

    this->declare_parameter<std::string>("yolo_model_path", "/capella/lib/python3.10/site-packages/wire_pipe_det/src/7_1_1.onnx");
    ...
    this->declare_parameter<double>("distance_log_throttle_sec", 3.0);
    ...
    this->declare_parameter<std::vector<double>>("distortion_coefficients",
        std::vector<double>{-0.40698, 0.22766, -0.00009, 0.00010, -0.07873});
    ...
    const sensor_msgs::msg::CompressedImage::SharedPtr msg)
{
    ...
    for (const auto &target : confirmed_targets) {
        ...
        geometry_msgs::msg::Point pt_laser = estimate3DFromLaser(
            det_bbox, frame.cols, dist_laser, laser_frame_id);
